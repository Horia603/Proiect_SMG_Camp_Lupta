...
public:
	...
	void SetFloat(const std::string& name, float amb) const
	{
		glUniform1f(glGetUniformLocation(ID, name.c_str()), amb);
	}
...
int main(int arcg, char** argv)
{
	...
	// -------------------------
	Shader shadowMappingShader("PhongLight.vs", "PhongLight.fs");
	Shader shadowMappingDepthShader("Lamp.vs", "Lamp.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (spin == true)
		{
			...
			float theta = 2.0f * 3.1415926f * float(temp) / float(45);//get the current angle
...
void renderScene(const Shader& shader)
{
	...
	model = glm::rotate(model, 50.0f, glm::vec3(0.0f, 1.75f, 0.0f));
	shader.SetMat4("model", model);
	renderCube();

	//// cube
	//model = glm::mat4();
	//model = glm::translate(model, glm::vec3(0.0f, 1.75f, 3.0));
	//model = glm::scale(model, glm::vec3(0.75f));
	//model = glm::rotate(model, 13.0f, glm::vec3(0.0f, 1.75f, 3.0f));
	//shader.SetMat4("model", model);
	//renderCube();

	//// cube
	//model = glm::mat4();
	//model = glm::translate(model, glm::vec3(6.0f, 2.75f, 1.0));
	//model = glm::scale(model, glm::vec3(0.75f));
	//model = glm::rotate(model, 32.0f, glm::vec3(6.0f, 2.75f, 1.0f));
	//shader.SetMat4("model", model);
	//renderCube();
